...
describe('allmaps iiif', () => {
  test('should display help when no arguments are provided', () => {
    expect(() => execFilename('iiif')).to.throw(helpMessage('iiif'))
  })

  test('should display help when incorrect command is provided', () => {
    expect(() => execFilename('iiif incorrect')).to.throw(
      helpMessage('iiif-incorrect')
    )
  })
})

describe('allmaps iiif manifest', () => {
  test('should read multiple IIIF Resources from the standard input and create a new Manifest that contains all Images from all input IIIF Resources', () => {
...
describe('allmaps iiif parse', () => {
  test("should read a IIIF Manifest from the standard input and convert the Manifest to Allmaps' internal format", () => {
