<script lang="ts">
  /* eslint-disable svelte/no-navigation-without-resolve -- annotation and viewer links are external */
  ...
  import type { LanguageString, ListItem } from '$lib/types.js'
  ...
  let username = $derived(page.data.session?.user?.slug ?? null)
  let listUrl = $derived(
  ...
  async function removeItem(item: ListItem) {
    ...
    try {
      await removeListItemCommand({
        listId,
        mapId: item.mapId ?? undefined,
        imageId: item.imageId ?? undefined,
        canvasId: item.canvasId ?? undefined,
        manifestId: item.manifestId ?? undefined
      })
      await invalidateAll()
    } finally {
      removingItem = null
