<script lang="ts">
  ...
  function cancel() {
    void goto(data.isCurrentUser ? routes.home() : routes.users())
  }
</script>
      ...
      <div class="space-y-3">
        <div>
          ...
          <p class="text-sm font-mono text-gray-600">
            {data.session?.session?.id || 'N/A'}
          </p>
        </div>
        <div>
          <div class="text-sm font-medium text-gray-500">Expires At</div>
          <p class="text-lg">
            {data.session?.session?.expiresAt
              ? new Date(data.session.session.expiresAt).toLocaleString()
              : 'N/A'}
          </p>
        </div>
      </div>
