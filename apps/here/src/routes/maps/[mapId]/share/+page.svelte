<script lang="ts">
  ...
  function getPostcardText(postcardUrl?: string) {
    return `Look, I'm here on this map${locality ? ` of ${locality}` : ''}${year ? ` from ${year}` : ''}! Where are you?${postcardUrl ? ` ${postcardUrl}` : ''}`
  }
  ...
  <Dialog.Portal>
    ...
    >
      ...
      <div class="text-center text-sm text-gray-600 max-w-md">
        Copy the message or the link to send this postcard to your friends using
        apps like Signal, Slack, or Bluesky.
      </div>
