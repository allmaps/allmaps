...
<Popover.Root bind:open>
  <Popover.Trigger {disabled}>
    {#snippet child({ props })}
      ...
      >
        <DropIcon class="size-full" />
        {#if opacityHidden}
          <span
            class="pointer-events-none absolute h-1 w-5 rotate-45 rounded-full border border-white bg-pink group-disabled:bg-gray pointer-coarse:w-7"
