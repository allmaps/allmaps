<script lang="ts">
  ...
  $effect(() => {
    ...
    warpedMapLayer.setOptions(
      deleteFromAnnotationOptions(optionsState?.mergedOptions)
    )
    if (optionsState?.mergedOptions.opacity !== undefined) {
      warpedMapLayer?.setOpacity(optionsState?.mergedOptions.opacity)
    }
    const transformationType = deleteFromAnnotationOptions(
      optionsState.mergedOptions
    ).transformationType
    console.log(transformationType)
    if (transformationType) {
      // TODO: allow for setting transformation type from annotation
      warpedMapLayer.setMapsTransformationType(mapIds, transformationType)
    }
    const internalProjection = deleteFromAnnotationOptions(
      optionsState.mergedOptions
    ).internalProjection
