...
export default function createWarpedMapLayerClass<
  WarpedMapSource extends BaseWarpedMapSource,
  FrameState extends BaseFrameState
>(
  BaseLayer: Class,
  ViewHint: BaseViewHint,
  OLWarpedMapEvent: BaseOLWarpedMapEvent
) {
  const WarpedMapLayer = class extends BaseLayer {
    ...
    #lastPreparedFrameLayerRevision = 0
    #lastPreparedFrameSourceRevision = 0

    #previousExtent: number[] | null = null

    /**
     * Create a WarpedMapLayer.
    ...
    #prepareFrameInternal(frameState: FrameState) {
      ...
      if (this.source) {
        sourceChanged =
          this.#lastPreparedFrameSourceRevision < this.source.getRevision()
