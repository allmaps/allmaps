...
export class Projective extends BaseTransformation {
  ...
  constructor(sourcePoints: Point[], destinationPoints: Point[]) {
    ...
    // Hence projectiveCoefsMatrix and projectiveWeightsMatrix are one Matrix

    // Construct 2Nx9 Matrix projectiveCoefsMatrix
    // −x0 −y0 −1  0   0   0  x'0x0 x'0y0 x'0
    // 0   0   0   −x0 −y0 −1 y'0x0 y'0y0 y'0
    // ...
    this.coefsMatrix = Matrix.zeros(2 * this.pointCount, 9)
    for (let i = 0; i < this.pointCount; i++) {
      this.coefsMatrix.set(2 * i, 0, -sourcePoints[i][0])
      this.coefsMatrix.set(2 * i, 1, -sourcePoints[i][1])
      this.coefsMatrix.set(2 * i, 2, -1)
      this.coefsMatrix.set(2 * i, 3, 0)
      this.coefsMatrix.set(2 * i, 4, 0)
      this.coefsMatrix.set(2 * i, 5, 0)
      ...
      this.coefsMatrix.set(2 * i, 8, destinationPoints[i][0])
      this.coefsMatrix.set(2 * i + 1, 0, 0)
      this.coefsMatrix.set(2 * i + 1, 1, 0)
      this.coefsMatrix.set(2 * i + 1, 2, 0)
      this.coefsMatrix.set(2 * i + 1, 3, -sourcePoints[i][0])
      this.coefsMatrix.set(2 * i + 1, 4, -sourcePoints[i][1])
      this.coefsMatrix.set(2 * i + 1, 5, -1)
      this.coefsMatrix.set(
        2 * i + 1,
        6,
      ...
      this.coefsMatrix.set(2 * i + 1, 8, destinationPoints[i][1])
    }
  ...
  solve() {
    ...
    const svdCoefsMatrix = new SingularValueDecomposition(this.coefsMatrix)

    this.weightsMatrix = Matrix.from1DArray(
      3,
      3,
      svdCoefsMatrix.rightSingularVectors.getColumn(8)
    ).transpose()
    this.weights = this.weightsMatrix.to2DArray()
  }
  ...
  evaluateFunction(newSourcePoint: Point): Point {
    ...
    const c =
      this.weights[0][2] * newSourcePoint[0] +
      this.weights[1][2] * newSourcePoint[1] +
      this.weights[2][2]
    const num1 =
      this.weights[0][0] * newSourcePoint[0] +
      this.weights[1][0] * newSourcePoint[1] +
      this.weights[2][0]
    const num2 =
      this.weights[0][1] * newSourcePoint[0] +
      this.weights[1][1] * newSourcePoint[1] +
      this.weights[2][1]
  ...
  evaluatePartialDerivativeX(newSourcePoint: Point): Point {
    ...
    const newDestinationPointPartDerX: Point = [
      (c * this.weights[0][0] - this.weights[0][2] * num1) / c ** 2,
      (c * this.weights[0][1] - this.weights[0][2] * num2) / c ** 2
    ]
  ...
  evaluatePartialDerivativeY(newSourcePoint: Point): Point {
    ...
    const newDestinationPointPartDerY: Point = [
      (c * this.weights[1][0] - this.weights[1][2] * num1) / c ** 2,
      (c * this.weights[1][1] - this.weights[1][2] * num2) / c ** 2
    ]
