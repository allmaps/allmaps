...
import { gcps_3, gcps_6, gcps_10 } from './input/gcps-test.js'
...
describe('Helmert transformer', async () => {
  const transformer = new GCPTransformer(gcps_3, 'helmert')
  const input = [100, 100]
...
describe('Polynomial transformer, order 1', async () => {
  const transformer = new GCPTransformer(gcps_3, 'polynomial')
  const input = [100, 100]
...
describe('Polynomial transformer, order 2', async () => {
  const transformer = new GCPTransformer(gcps_6, 'polynomial2')
...
describe('Polynomial transformer, order 3', async () => {
  const transformer = new GCPTransformer(gcps_10, 'polynomial3')
...
describe('Projective transformer', async () => {
  const transformer = new GCPTransformer(gcps_6, 'projective')
  const input = [1000, 1000]
...
describe('Thin-plate spline transformer', async () => {
  const transformer = new GCPTransformer(gcps_6, 'thin-plate-spline')
  const input = [1000, 1000]
